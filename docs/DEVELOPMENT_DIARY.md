...
The decision was conservative: Terraform manages only the `music-platform` namespace and its baseline labels (`istio-injection=enabled`). Everything generated by Helm stays exclusively in Helm. This boundary is the core of all four Terraform documentation files.
